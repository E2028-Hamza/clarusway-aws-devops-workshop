...
- Change the service type of web-svc service to NodePort to use the Node IP and a static port to expose the service outside the cluster. So we get the yaml file below.
...
```bash
kubectl delete -f service-lessons
