...
- name: Apache installation for webservers
  hosts: webservers
  become: true        # bu satırı eklediğimiz için <ansible-playbook "-b" playbook4.yml> komutunda -b kullanMAdık
  tasks:
...
$ ansible-playbook playbook4.yml   # Run the command again and show the changing parts of the output.
```
...
```bash
ansible node1 -m setup | grep dist
```
